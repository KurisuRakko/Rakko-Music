...
import { ListMusic, Settings as SettingsIcon, Disc, Mic2, Music2, Pause, Play, Upload, FileMusic, Maximize2 } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // --- Presentation Sync ---
  usePresentationSync({
    role: isPresentationMode ? 'follower' : 'leader',
    state: {
      currentSong,
      audioState,
      currentCover
    ...
    onSync: (payload) => {
      ...
      if (payload.currentSong) {
        // Only update song if ID changed to avoid unnecessary re-renders or loops
        setCurrentSong(prev => prev?.id === payload.currentSong?.id ? prev : payload.currentSong);
      } else {
        setCurrentSong(null);
      }

      setCurrentCover(payload.currentCover);
      setAudioState(payload.audioState);

      // Setup simple animation loop for smooth progress in follower mode?
      // For now, rely on frequent updates from leader.
    }
  });
  ...
  const handleLoadedMetadata = () => {
    ...
    if (audioState.isPlaying && !isPresentationMode) {
  ...
  }, [handleEnded, audioState.volume, isPresentationMode]);
  ...
  return (
    ...
    >
      ...
      {isPresentationMode && (
        <div className="absolute inset-0 z-[9000] bg-black">
          {/* Reuse Background Logic */}
          <div className="absolute inset-0 z-0 overflow-hidden">
            <div
              className={`absolute inset-0 bg-cover bg-center transition-all duration-1000 ease-elegant opacity-100 scale-100`}
              style={{
          ...
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-12 md:p-24">
            <div className="flex flex-col md:flex-row w-full h-full max-w-7xl gap-12 items-center">
              {/* Left: Art & Info */}
              <div className="flex-1 flex flex-col items-center justify-center text-center max-w-md">
                <div className="w-full aspect-square rounded-[2rem] shadow-2xl overflow-hidden mb-8 border border-white/10 relative">
                  <div className="absolute inset-0 bg-cover bg-center transition-all duration-700" style={{ backgroundImage: currentCover ? `url(${currentCover})` : undefined, backgroundColor: '#222' }}>
                    {!currentCover && <div className="w-full h-full flex items-center justify-center"><Disc size={64} className="opacity-20" /></div>}
                  </div>
                </div>
                <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 drop-shadow-md leading-tight">
                  {currentSong?.metadata?.title || currentSong?.name || "Waiting for Music..."}
                </h1>
              ...
              {/* Right: Lyrics */}
              <div className="flex-[1.5] h-full overflow-hidden mask-linear-fade">
                <LyricsView
